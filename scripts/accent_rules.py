...
def apply_accent_rules(
    ...
    has_ir_preverb = preverb == "ir"

    # Rules A, B, C, D only apply to ir-preverb verbs ("to come" pattern).
    # Appendix 1 accented forms are almost exclusively from "to come" (ir- preverb).
    # The uur- preverb ("to be good") does NOT produce accented forms.
    # Non-ir-preverb verbs only get Rule A2 (potential cí from ut+i affrication).
    if not has_ir_preverb:
        # Rule A2 only — potential cí from ut+i affrication
        # Accent applies when short i after c is followed by ʔ (glottal stop),
        # indicating the potential stem pattern ucíʔaar. Does NOT apply to:
        # - 1sg (agent t absorbs the accent environment)
        # - Plural forms where ci is followed by raak (no ʔ)
        if 'potential' in mode and person_number != '1sg':
            result = list(form)
            for i in range(2, len(result)):
                if result[i] == 'c' and i + 1 < len(result) and result[i+1] in ('i', 'í'):
                    is_long = (i + 2 < len(result) and result[i+2] in ('i', 'í'))
                    has_glottal = (i + 2 < len(result) and result[i+2] == 'ʔ')
                    if not is_long and has_glottal:
                        result[i+1] = 'í'
            return ''.join(result)
        return form
